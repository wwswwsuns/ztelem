...
func (p *TelemetryParser) parseInterfaceCounters(msg *zteTelemetry.Telemetry) ([]models.InterfaceMetric, error) {
	...
	for _, dataGpb := range msg.DataGpb {
		...
		if countersList := interfaceInfo.GetCounters(); len(countersList) > 0 {
			...
			metric.InputUtilization = float64Ptr(utilizationToNumeric(counters.GetInputUtilization()))
			metric.OutputUtilization = float64Ptr(utilizationToNumeric(counters.GetOutputUtilization()))
			...
			metric.InputV4Utilization = float64Ptr(utilizationToNumeric(counters.GetInputV4Utilization()))
			metric.OutputV4Utilization = float64Ptr(utilizationToNumeric(counters.GetOutputV4Utilization()))
			metric.InputV6Utilization = float64Ptr(utilizationToNumeric(counters.GetInputV6Utilization()))
			metric.OutputV6Utilization = float64Ptr(utilizationToNumeric(counters.GetOutputV6Utilization()))
