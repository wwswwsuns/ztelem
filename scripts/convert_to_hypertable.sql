-- 将告警数据表转换为TimescaleDB超表并配置压缩和保留策略
-- 执行前请确保已备份数据！
...
LEFT JOIN timescaledb_information.hypertable_detailed_size(
...
JOIN timescaledb_information.hypertables h ON j.hypertable_schema = h.hypertable_schema 
    AND j.hypertable_name = h.hypertable_name
WHERE j.proc_name = 'policy_compression'
  AND h.hypertable_schema = 'telemetry'
